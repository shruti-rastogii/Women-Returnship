...
const CourseInfo = (props) => {
    ...
    return <div style={{display:"flex", flexDirection:"row", margin:"1rem"}}>
      ...
      <div style={{margin:"2rem", fontSize:"20px", marginTop:"0rem"}}>Organization: {props.item.organization}</div>
      <div style={{margin:"2rem", fontSize:"20px", marginTop:"0rem"}}>Type: {props.item.type}</div>
      <div style={{margin:"2rem", fontSize:"20px", marginTop:"0rem"}}>Duration: {props.item.duration}</div>
      <div style={{margin:"2rem", fontSize:"20px", marginTop:"0rem"}}>Price: {props.item.price}</div>
      <div style={{margin:"2rem", fontSize:"20px", marginTop:"0rem"}}>Curriculum: {props.item.curriculum.map((item)=>{return <><br/>{item}</>})}</div>
      <div style={{margin:"2rem", fontSize:"20px", marginTop:"0rem"}}>Description: {props.item.Description}</div>
      </div>
    </div>
}
